...
public class CercaDataStrategy implements IRicercaMessaggiStrategy {
	...
	@Override
	public List<Messaggio> cerca(List<Messaggio> conversazione, String dataCercata) {
		List<Messaggio> filtrati = new ArrayList<>();
		if (conversazione != null && !conversazione.isEmpty() && !dataCercata.isBlank()) {
			try {
				LocalDate dataUtente = LocalDate.parse(dataCercata.trim(), FORMATO_INPUT);
